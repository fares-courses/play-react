...
1. **Lazy components** — `lazy(() => import("./Foo"))` suspends while the chunk loads (you saw this in doc 09).
2. **Suspense-enabled data fetching.** TanStack Query has a Suspense mode, and React 19's `use()` hook (doc 13) suspends on unresolved promises.
...
`useTransition` (we'll cover properly in doc 13) marks an update as low-priority and tells React to keep showing the old UI:
...
6. What does `useTransition` (doc 13) help with in the context of route or filter changes?
...
- **Doc 13** — React 19 features. `use()`, `useTransition`, `useOptimistic`, `useActionState` — many of which complement Suspense.
- **Doc 14** — Server Components, where Suspense becomes first-class and the patterns scale up.
