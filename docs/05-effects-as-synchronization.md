...
**5. Fetching data in most cases** (use TanStack Query, doc 10). The effect-based fetch pattern is full of footguns: race conditions, stale closures, no caching, no deduplication, no retry. You can write it once correctly to learn, but in real apps, use a library.
...
- Memoize it with `useMemo`/`useCallback` (doc 08).
...
In real apps you let TanStack Query (doc 10) handle this for you. But you should know it exists, so you can spot the bug in code that doesn't use a library.
...
- *"Replace this effect-based fetch with a TanStack Query call (doc 10 territory). Keep the loading and error states."*

## Where this goes next

- **Doc 06** — Refs, the imperative escape hatch when effects/state aren't the right tool.
- **Doc 10** — TanStack Query, which makes most fetch-in-effect code obsolete.
- **Doc 12** — Suspense, the modern way to handle loading states without manual flags.
