# 17 — Server Components mental model
...
The client form calls the server function as if it were local. Behind the scenes, the framework serializes the call to an HTTP request. From the developer's perspective, you're just calling a function. Pairs naturally with `useActionState` (doc 16).
...
**1. Conceptual quiz:**
```
Quiz me on RSC concepts from doc 17. Cover:
...
- **Doc 18** — Testing. Same principles whether SPA or RSC.
- **Doc 19** — Tooling and shipping. Vite, accessibility, bundle awareness.
- **Doc 20** — Auth flows. Differs slightly in RSC (cookies + middleware) vs SPA (Bearer tokens) — we'll do SPA-style and note the RSC variant.
